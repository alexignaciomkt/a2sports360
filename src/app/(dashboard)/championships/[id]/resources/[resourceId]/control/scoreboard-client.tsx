...
// Catálogo de Eventos (ScoreAction)
...
export function ScoreboardClient({
  ...
  championshipId,
  resourceId,
  resourceDisplayName = "Local Operacional"
}: ScoreboardProps) {
  ...
  // Supabase Realtime for the resource control
  useEffect(() => {
  ...
  async function handleScore(teamId: string, points: number) {
    ...
    const res = await registerPoint(championshipId, resourceId, teamId, points, deviceId)
    if (!res.success) {
      setError(res.error!)
      if (res.expired) setControlState('expired')
    } else if (res.state && typeof res.state === 'object') {
      setMatch((current: Record<string, unknown>) => ({ ...current, ...(res.state as object) }))
    }
    setLoading(false)
  }
  ...
  async function handleUndo() {
    ...
    const res = await undoLastPoint(championshipId, resourceId)
    if (!res.success) {
      setError(res.error!)
      if (res.expired) setControlState('expired')
    } else if (res.state && typeof res.state === 'object') {
  ...
  function renderTeamCard(teamId: string, teamName: string, currentScore: number, isWinner: boolean, colorClass: 'indigo' | 'rose') {
    ...
    // Multi-modality preparation: Catálogo de Eventos da modalidade
    const scoreActions: ScoreAction[] = [
      { id: "NORMAL", label: "+1", delta: 1, analyticsKey: "normal", isPrimary: true },
      { id: "TRUCO", label: "+3", delta: 3, analyticsKey: "truco" },
      { id: "SEIS", label: "+6", delta: 6, analyticsKey: "seis" },
      { id: "NOVE", label: "+9", delta: 9, analyticsKey: "nove" },
      { id: "DOZE", label: "+12", delta: 12, analyticsKey: "doze" }
    ]
    const primaryAction = scoreActions.find(a => a.isPrimary) || scoreActions[0]
    const secondaryActions = scoreActions.filter(a => a !== primaryAction)
    ...
    return (
      <div className={`flex-1 rounded-[2rem] border border-zinc-800/80 bg-[#0a0a0c] p-6 sm:p-8 flex flex-col items-center justify-start relative overflow-hidden shadow-2xl transition-colors duration-500`}>
        ...
        <div className="flex items-center justify-center gap-6 sm:gap-12 mb-auto w-full z-10">
          ...
          <button
            onClick={() => handleScore(teamId, primaryAction.delta)}
            ...
            title={`Adicionar ${primaryAction.label}`}
          >
            {primaryAction.delta === 1 ? (
               <Plus className="h-10 w-10 sm:h-12 sm:w-12 stroke-[2.5]" />
            ) : (
               <span className="text-3xl font-black">{primaryAction.label}</span>
            )}
          </button>
        </div>
        
        {/* Presets Row: Score Event Actions */}
        {secondaryActions.length > 0 && (
          <div className="w-full mt-10">
            <div 
              className="grid gap-2 sm:gap-3 w-full"
              style={{ gridTemplateColumns: `repeat(${secondaryActions.length}, minmax(0, 1fr))` }}
            >
              {secondaryActions.map(action => {
                const isDisabled = loading || isFinished;
                
                return (
                  <button
                    key={action.delta}
                    onClick={() => handleScore(teamId, action.delta)}
                    disabled={isDisabled}
                    className={`flex flex-col items-center justify-center py-3 sm:py-4 rounded-[1rem] transition-transform active:scale-95 disabled:opacity-30 disabled:hover:scale-100 shadow-lg border
                      ${isDisabled ? 'bg-zinc-900 border-zinc-800 text-zinc-600' : 'bg-zinc-800 border-zinc-700 text-zinc-300 hover:bg-zinc-700'}
                    `}
                  >
                    <span className="font-black text-2xl sm:text-3xl leading-none">{action.label}</span>
                  </button>
                )
              })}
            </div>
  ...
  if (controlState === 'missing') {
    return (
      <div className="flex flex-col items-center justify-center p-12 text-center h-full">
        <h2 className="text-2xl font-bold text-white mb-4">Local Livre</h2>
        <p className="text-zinc-400 mb-8 max-w-md">Não há nenhuma sessão de controle ativa para este Local. Assuma o controle para iniciar a pontuação.</p>
        <button
          onClick={async () => {
            setLoading(true)
            const { startControlSession } = await import('./actions')
            const res = await startControlSession(championshipId, resourceId)
            if (res.success) setControlState('active')
            setLoading(false)
          }}
          disabled={loading}
          className="px-6 py-3 bg-indigo-600 hover:bg-indigo-500 disabled:opacity-50 text-white font-bold rounded-lg transition-colors"
        >
          {loading ? 'Iniciando...' : 'Iniciar Controle'}
        </button>
      </div>
    )
  }
  ...
  if (controlState === 'occupied_by_another_session') {
    return (
      <div className="flex flex-col items-center justify-center p-12 text-center h-full">
        <h2 className="text-2xl font-bold text-rose-500 mb-4">Local Ocupado</h2>
        <p className="text-zinc-400 mb-8 max-w-md">Outro dispositivo já está controlando este Local. Você pode forçar a tomada de controle se necessário.</p>
        <button
          onClick={async () => {
            setLoading(true)
            const { takeOverControlSession } = await import('./actions')
            const res = await takeOverControlSession(championshipId, resourceId)
            if (res.success) setControlState('active')
            setLoading(false)
          }}
          disabled={loading}
